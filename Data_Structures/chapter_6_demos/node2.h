...
namespace main_savitch_6B
{
    ...
    public:
        ...
        Item& data( ) { return m_data_field; }
        node* link( ) { return m_link_field; }
        void set_data(const Item& new_data) { m_data_field = new_data; }
        void set_link(node* new_link) { m_link_field = new_link; }
        // CONST MEMBER FUNCTIONS
        const Item& data( ) const { return m_data_field; }
        const node* link( ) const { return m_link_field; }
    private:
        Item m_data_field;
        node *m_link_field;
    };
	...
	node_iterator operator ++(int) // Postfix ++
	    {
		node_iterator original(m_current);
		m_current = m_current->link( );
		return original;      	  
	    }
	...
	const_node_iterator operator ++(int) // Postfix ++
	    {
		const_node_iterator original(m_current);
		m_current = m_current->link( );
		return original;
	    }
