class HerbivoreMove implements Movable {
    ...
    public HerbivoreMove(Environment env, int x, int y) {
	this.env = env;
	this.x = x;
	this.y = y;
	...
	if(env.moveAgent(x, y, xNew, yNew)) {
	    x = xNew;
	    y = yNew;
	    mofk().setEnergy(mofk().getEnergy() + 1.0);
	...
	if (mofk().getAge() > 3 && mate.getAge() > 3 && mofk().getEnergy() > 6 && mate.getEnergy() > 6) {
		...
		earth[childSpace[0]][childSpace[1]] = new Herbivore(childSpace[0], childSpace[1], env);
	    }
	...
	prey.die();
	mofk().setEnergy(mofk().getEnergy()+4); // yum
    }
